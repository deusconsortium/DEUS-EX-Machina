...
def f(x):
    return (x ** 3. - 3. * x) * (erf(sqrt(5. / 2.) * x) + erf(sqrt(5. / 2.) * x / 2.)) / 2. + sqrt(2. / (5. * pi)) * (
    (31. * x ** 2. / 4. + 8. / 5.) * exp(-5. * x ** 2. / 8.) + (x ** 2. / 2. - 8. / 5.) * exp(-5. * x ** 2. / 2.))


def I_dn_dr1(x):
    return -(
    -(8. * x ** 4. + 2. * x ** 2. + 5.0) / (x ** 4. * (5. + 4 * x ** 2.) ** 2.) * 9. * sqrt(5. / (2. * pi)) + 3. * (
    x ** 2. - 1.) / (x ** 5. * sqrt(2. * pi)) * (arctan(sqrt(5.) / x) + arctan(sqrt(5.) / (2. * x))))


def Wg(a, b):
    return exp(-(a + b) ** 2. / 2.) - exp(-(a - b) ** 2. / 2.) + b / 2. * sqrt(2. * num.pi) * (
    erf((a - b) / sqrt(2.)) + erf((a + b) / sqrt(2.)))
...
def In(y, n, x0=None):
    ...
    if x0 is None:
        ...
        return 2. ** (n / 2. - 2.) * gamma((1. + n) / 2.) / (5. * sqrt(5. * num.pi)) * (
            ...
            + 25. * (1. + n) * (3. + n) * x ** (-(n + 5.)) * hyp2f1(1. / 2., (5. + n) / 2., 3. / 2.,
                                                                    -5. / (4. * x ** 2.)))
    else:
...
class file_tools:
    @staticmethod
    def select_file(folder_path, nb_char_to_remove=None):
        ...
        if 0 <= number < size(folder):
            if nb_char_to_remove is not None:
                return folder[number][:-nb_char_to_remove]
            else:
    ...
    @staticmethod
    def list_not_hidden_directory(dir_path):
        flist = []
    ...
    @staticmethod
    def get_files_that_end_as(dir_path, end, end_char_to_cut=0):
...
def get_mass_contrast(r, d):
    ...
    for i in range(size(d) - 1):
        if i == 0:
            ...
            x = ru[i] / ru[i + 1]
            f[i + 1] = f[i] * x ** 3. + 1. / 4. * (1 - x) * (
            x ** 2. * (d[i + 1] + 3. * d[i]) + 2. * x * (d[i + 1] + d[i]) + d[i] + 3. * d[i + 1])
...
def get_density(r, f, method='simple'):
    d = num.ones(size(r))
    ...
    if method == 'defaut':
        ...
        for i in range(size(r) - 2):
            j = size(r) - 3 - i
            x = r[j] / r[j + 1]
            d[j] = 4. / 3. * (f[j + 1] - x ** 3. * f[j]) / ((1. - x) * (1. + x ** 2.)) - d[j + 1] / 3. * (
            5. + x ** 2.) / (1. + x ** 2.)
...
class physics_tools:
    @staticmethod
    def epsilon0(w, wm0, z_init):
        if wm0 == 1.0:
            return 0.0
        return (1. - wm0) / wm0 * (z_init + 1.) ** (3. * w)

    @staticmethod
    def omega_m_0(wm_today, w, z_init):
        x = 1. / (z_init + 1.0) ** (3. * w)
        return wm_today * x / (1. - wm_today * (1. - x))

    @staticmethod
    def omega_m(alpha, wm0, w, z_init):
        return 1. / (1. + physics_tools.epsilon0(w, wm0, z_init) * alpha ** (-3. * w))

    @staticmethod
    def eta(a, w, wm0, d_logD_d_loga, z_init):
        s0 = d_logD_d_loga
        alpha = a * (z_init + 1.)
        ...
        if wm0 == 1.:
            ...
            e0 = physics_tools.epsilon0(w, wm0, z_init)

            eta1 = hyp2f1(1. - 1. / (3. * w), 1. + 1. / (2. * w), (9. + 1. / w) / 6., -1. / e0)
            eta2_a = hyp2f1(1. / 2. + 1. / (3. * w), (w - 1.) / (2. * w), 3. / 2. - 1. / (6. * w),
                            -math.pow(alpha, 3. * w) / e0)
            ...
            zeta_a = math.pow(alpha, (3. * w - 1.) / 2.) * eta2_a * (9. * w - 1.) * (
            6. * eta1 - 2. * e0 * s0 * eta4_1 * (3. * w + 1.)) + 3. * (1. + 3. * w) * (w - 1.) * (
            2. + 3. * w) * eta3 * eta4_a + e0 * (1. + 3. * w) * (1. + 2. * s0 - 3. * w) * (9. * w - 1.) * eta2_1 * eta4_a
            zeta_1 = eta2_1 * (9. * w - 1.) * (6. * eta1 - 2. * e0 * s0 * eta4_1 * (3. * w + 1.)) + 3. * (1. + 3. * w) * (
            w - 1.) * (2. + 3. * w) * eta3 * eta4_1 + e0 * (1. + 3. * w) * (1. + 2. * s0 - 3. * w) * (
            9. * w - 1.) * eta2_1 * eta4_1
...
class SplineFit:
    ...
    def __call__(self, x):
        if size(x) > 1:
            ...
            if index >= 0:
                ...
                return self._c[index][0] + self._c[index][1] * X + self._c[index][2] * X ** 2. + self._c[index][
                                                                                                     3] * X ** 3.
            else:
                return None
    ...
    def Integrate(self, x0=None, x1=None):
        ...
        for j in range(size(self._x) - 1):
            ...
            I += (self._x[j + 1] - self._x[j]) * (
            self._c[j][0] * X + self._c[j][1] * X ** 2. / 2. + self._c[j][2] * X ** 3. / 3. + self._c[j][
                3] * X ** 4. / 4.)
        return I
    ...
    def Primitive(self):
        ...
        for j in range(size(self._x) - 1):
            I[j + 1] = I[j] + (self._x[j + 1] - self._x[j]) * (
            self._c[j][0] + self._c[j][1] / 2. + self._c[j][2] / 3. + self._c[j][3] / 4.)
        return I
...
class plot_tools:
    ...
    @staticmethod
    def error_plot(x, y, dy, linestyle='-', color='b', marker='o', label=None):
...
class integrate_tools:
    @staticmethod
    def alpha_to_y(w, e0, alpha):
        if e0 == 0.:
            return num.log(alpha) / num.sqrt(2.)
        else:
            return num.sqrt(2.) / (3. * w) * (num.arcsinh(num.power(alpha, 3. * w / 2.) / num.sqrt(e0))) - num.arcsinh(
                1. / num.sqrt(e0))
    ...
    def y_to_alpha(w, e0, y):
        if e0 == 0.:
            ...
            return math.pow(e0, 1. / (3. * w)) * math.pow(
                num.sinh(num.arcsinh(1. / num.sqrt(e0)) + 3. * w * y / num.sqrt(2.)), 2. / (3. * w))
    ...
    def dynamic_y(v, t, param):
        w, e0, f0 = param
        return [v[1], -v[1] / (num.sqrt(2.) * integrate_tools.sqrt_omega_m_y(w, e0, t)) + v[0] - f0 * math.pow(v[0], -2)]

    @staticmethod
    def evolve_psi_linear(f0, a_tab, w, Wm0, zcmb, s0):
        if f0 == 1.:
            if size(a_tab) > 1:
        ...
        n = physics_tools.eta(a_tab, w, Wm0, s0, zcmb)
        return 1.0 - (f0 - 1.) * n, 0.0

    @staticmethod
    def evolve_psi(f0, a_tab, w, Wm0, zcmb, s0):
        if f0 == 1.:
            if size(a_tab) > 1:
                psi = num.empty(size(a_tab))
                for i in range(size(psi)):
        ...
        e0 = physics_tools.epsilon0(w, Wm0, zcmb)
        ...
        params = [w, e0, f0]
        t = integrate_tools.alpha_to_y(w, e0, alpha_tab)
        sol = Nintegrate.odeint(integrate_tools.dynamic_y, y0, t, args=(params,))
        retour, v = sol.T
        retour = num.delete(retour, 0)
        v = num.delete(v, 0)

        a = alpha_tab[size(alpha_tab) - 1]
        return retour, v / sqrt(2. * physics_tools.omega_m(a, Wm0, w, zcmb))

    @staticmethod
    def evolve_profile(r0, f0, af, w, Wm0, zcmb, dlogD_dloga_cmb):
        ...
        if size(af) == 1:
            ...
            for j in range(size(r0)):
                psi, v[j] = integrate_tools.evolve_psi(f0[j], af, w, Wm0, zcmb, s0)
                r[j] = r0[j] * psi
                if 1.0e-2 < psi < 1.0e+5:
                    f[j] = f0[j] / (psi ** 3.)
                else:
                    f[j] = None
        else:
            ...
            for i in range(size(r0)):
                psi, v[i] = integrate_tools.evolve_psi(f0[i], af, w, Wm0, zcmb, s0)
