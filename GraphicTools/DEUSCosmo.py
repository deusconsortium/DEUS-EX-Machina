...
class DEUSCosmo:
    ...
    def set_power_spectrum_path(self, path):
        self._power_spectrum_path = path

    def _set_simulation(self, boxlen, npart, cosmo):
        self._boxlen = boxlen
        self._npart = npart
        ...
        if cosmo == "lcdmw5":
            ...
            self._H0 = 0.72
            self._w = -1.0
        self._Wm0 = physics_tools.omega_m_0(self._WmToday, self._w, self._zinit)
    ...
    def _load_spectrum(self):
        if self._cosmo is None:
            ...
            founded = True
            try:
                k, P = num.loadtxt(self._power_spectrum_path + 'pk_' + self._cosmo + '.dat', usecols=(0, 1),
                                   unpack=True)
                a, D, dD = num.loadtxt(self._power_spectrum_path + 'mpgrafic_input_' + self._cosmo + '.dat',
    ...
    def _gaussian_smoothing(self, R):
        self._P0 = self._P0 * exp(-(self._k * R) ** 2.)

    def _numerical_smoothing(self):
