...
class DEUSDensity(DEUSExtremaGraphics,DEUSAnalytics):
	...
	def PlotPDF(self):
		...
		title("density PDF at z="+str(1./(self._a) - 1.)+" for boxlen"+str(self._boxlen)+"_n"+str(self._npart)+" Rs = "+str(self._Rg)+" [Mpc/h]")
		legend()
		...
		xlabel('$\\delta$')
		ylabel('PDF')
		title("extremum PDF at z="+str(1./(self._a) - 1.)+" for boxlen"+str(self._boxlen)+"_n"+str(self._npart)+" Rs = "+str(self._Rg)+" [Mpc/h]")
		...
		dtheo_evo_cmb, Ndtheo_evo_cmb = DEUSAnalytics.computeEvolvedExtremumDistributionFromHeigh(self,self._w,self._Wm0,1./self._a - 1.,zinit = 1100.)
