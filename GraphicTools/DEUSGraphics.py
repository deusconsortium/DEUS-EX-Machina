...
class DEUSGraphics(DEUSCosmo):
    ...
    def Load(self, file_name=None):
        if file_name is None:
            DEUSGraphics.Load(self, file_tools.select_file(self._dataPath, 12))
        else:
            ...
            if File is None:
                ...
                (self._boxlen, self._npart, self._isOverDensity, self._Nprofile, self._Nradius) = struct.unpack(
                    "<ii?ii", data[16 + Nc:33 + Nc])
                ...
                for i in range(self._Nprofile):
                    ...
                    self._f_tab[i] = num.asarray(struct.unpack("f" * (self._Nradius), data[
                                                                                      cursor0 + i * dc + 16: cursor0 + i * dc + 16 + 4 * self._Nradius]))
                    self._v_tab[i] = num.asarray(struct.unpack("f" * (self._Nradius), data[
                                                                                      cursor0 + i * dc + 16 + 4 * self._Nradius: cursor0 + i * dc + 16 + 8 * self._Nradius]))
                    ...
                    r1 = self._getR1(self._r, self._f_tab[i])
                    d = get_density(self._r, self._f_tab[i])
                    # r1_d = solve(self._r, d, 1.0)
                    r1_d = self._getR1(self._r, d)
                    self._r1_full[i] = r1
    ...
    def getMeanProfile(self, R1, Dr1="dr", select_density=False, r1d_factor=0.5):
        if Dr1 == 'dr':
        ...
        mask_m = num.zeros(size(self._r1_full))
        for i in range(size(self._r1_full)):
            if self._r1_full[i] is not None and R1 - Dr1 <= self._r1_full[i] <= R1 + Dr1:
                mask_m[i] = 1

        if select_density:
            r1dm = ma.masked_array(self._r1d_full, num.logical_not(num.logical_and(mask_m, dNone))).mean()

            mask_d = num.logical_and(
                ma.masked_inside(self._r1d_full, max(r1dm - r1d_factor * Dr1, 0.0), r1dm + r1d_factor * Dr1).mask,
                mNone)
    ...
    def PlotMeanProfile(self, R1value, Dr1='dr', Rsmooth=0.0, velocity=False, Precision=2):
        if Dr1 == 'dr':
        ...
        if N > 0:
            ...
            plot(r, get_density(r, f), linestyle='--', color='b', label='density')
