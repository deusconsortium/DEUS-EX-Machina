...
void particle_project(paramfile &params, vector<particle_sim> &p,
  ...
  // JPA : constants for luminosity reduction
  float maxDOF = params.find<float>("maxDOF", 0); // Get Depth of Field
  float coeffLuminosity = 20.0f; // Arbitrary coef to adjust effect  
  // Calculate shift to force I= 0.0 at Z = maxDOF
  float coeffLuminosityShift = 1.0 / ((1.0+(maxDOF*coeffLuminosity)) * (1.0+(maxDOF*coeffLuminosity))); 
...
#pragma omp for schedule(guided,1000)
  for (m=0; m<npart; ++m)
    ...
    if (!projection)
      ...
      //std::cout << "ICI z=" << p[m].z << " - I=" << p[m].I << std::endl;
      
      // JPA : reduce luminosity by 1/Z^2
      p[m].z *= coeffLuminosity; // Coef arbitraire...
      if(p[m].z > (maxDOF*coeffLuminosity)) {
          p[m].I = 0.0;
      }
      else if(p[m].z > 0.0) { // Output.Pos.z > 0.0) {		
		p[m].I = p[m].I / ((1.0+p[m].z) * (1.0+p[m].z)) - coeffLuminosityShift;    // L = 1/R^2                
                //std::cout << "positif z=" << p[m].z << " - I=" << p[m].I << std::endl;
      }
